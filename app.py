...
def simulate():
    try:
        ...
        if PosElectrodeThickness is not None and PosElectrodeThickness != 0:
            parameters.update(
                {"Positive electrode thickness [m]": PosElectrodeThickness}
            )
        ...
        for c_rate in c_rates:
            ...
            sim = pybamm.Simulation(
                model, parameter_values=parameters, experiment=experiment1
            )
            ...
            exp1graphs.append(
                {
                    ...
                    "values": sol["Throughput capacity [A.h]"].entries.tolist(),
                }
            )
            exp1graphs.append(
                {
        ...
        for c_rate in c_rates:
            ...
            sim = pybamm.Simulation(
                model, parameter_values=parameters, experiment=experiment2
            )
            ...
            exp2graphs.append(
                {
                    ...
                    "values": sol["Discharge capacity [A.h]"].entries.tolist(),
                }
            )
            exp2graphs.append(
                {
        ...
        sim = pybamm.Simulation(
            model, parameter_values=parameters, experiment=experiment3
        )
        ...
        exp3graphs.append(
            {
                "name": "Discharge capacity [A.h]",
                "fname": f"Capacity",
                "values": average_array(
                    sol["Discharge capacity [A.h]"].entries.tolist()
                ),
            }
        )
